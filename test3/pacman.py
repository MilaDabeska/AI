...
def forward(x, y, direction):
    if direction == 'istok':
        if x < 10 and [x + 1, y] not in obstacles_list:
            x += 1
    ...
    if direction == 'sever':
        if y < 10 and [x, y + 1] not in obstacles_list:
            y += 1
        return x, y, direction
...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        x_man, y_man = state[0], state[1]
        direc = state[2]
        dots = state[3]

        x_pac, y_pac, new_direction = forward(x_man, y_man, direc)
        if x_pac != x_man or y_pac != y_man:
            successors['ProdolzhiPravo'] = (x_pac, y_pac, new_direction,
                                            tuple([i for i in dots if i[0] != x_pac or i[1] != y_pac]))

        x_pac, y_pac, new_direction = forward(x_man, y_man, direc)
        if x_pac != x_man or y_pac != y_man:
            successors['ProdolzhiNazad'] = (x_pac, y_pac, new_direction,
                                            tuple([i for i in dots if i[0] != x_pac or i[1] != y_pac]))

        x_pac, y_pac, new_direction = forward(x_man, y_man, direc)
        if x_pac != x_man or y_pac != y_man:
            successors['SvrtiLevo'] = (x_pac, y_pac, new_direction,
                                       tuple([i for i in dots if i[0] != x_pac or i[1] != y_pac]))

        x_pac, y_pac, new_direction = forward(x_man, y_man, direc)
        if x_pac != x_man or y_pac != y_man:
            successors['SvrtiDesno'] = (x_pac, y_pac, new_direction,
                                        tuple([i for i in dots if i[0] != x_pac or i[1] != y_pac]))
    ...
    # @staticmethod
    # def mhd(pac, dot):
    #     return abs(pac[0] - dot[0]) + abs(pac[1] - dot[1])
    ...
    def h(self, node):
        ...
        for x, y in node.state[3]:
            tmp = Pacman.mhd((pac_x, pac_y), (x, y))
            dire[x, y] = tmp

        dire = sorted(dire.items(), key=lambda kv: (kv[1], kv[0]))
        if len(dire) != 0:
            final += dire[0][1]
        return final
...
if __name__ == "__main__":
    ...
    for i in range(0, number_dots):
        parts = input().split(',')
        position_dots = ((int(parts[0]), int(parts[1])))

    pacman = Pacman((x_position, y_position, direction, number_dots), obstacles_list)
