...
def down(snake, green, red):
    snakes = list(snake)
    green_app = list(green)
    red_app = list(red)
    ...
    if y - 1 >= 0 and (x, y - 1) not in red_app and (x, y - 1) not in snakes: #proverka za granicite na tablata
        new_curr = (x, y - 1)
        if new_curr in green_app:
            green_app.remove(new_curr)
            snakes.append(new_curr)
        else:
            snakes = snakes[1:]
            snakes.append(new_curr)

    return tuple(snakes), tuple(green_app)


def up(snake, green, red):
    snakes = list(snake)
    green_app = list(green)
    red_app = list(red)
    ...
    if y + 1 < 10 and (x, y + 1) not in red_app and (x, y + 1) not in snakes:
        new_head = (x, y + 1)
        if new_head in green_app:
            green_app.remove(new_head)
...
def right(snake, green, red):
    ...
    if x + 1 < 10 and (x + 1, y) not in red_app and (x + 1, y) not in snakes:
        new_head = (x + 1, y)
        if new_head in green_app:
            green_app.remove(new_head)
            snakes.append(new_head)
...
def left(snake, green, red):
    ...
    if x - 1 >= 0 and (x - 1, y) not in red_app and (x - 1, y) not in snakes:
        new_head = (x - 1, y)
        if new_head in green_app:
            green_app.remove(new_head)
            snakes.append(new_head)
...
class Snake(Problem):
    ...
    def successor(self, state):
        ...
        if direction == 'South':
            ...
            new_snake, new_green = down(snake, green, red)
            if new_snake != snake:
                successors['ProdolzhiPravo'] = (new_snake, new_direction, new_green)

        elif direction == 'North':
            new_direction = direction
            new_snake, new_green = up(snake, green, red)
            if new_snake != snake:
                successors['ProdolzhiPravo'] = (new_snake, new_direction, new_green)

        elif direction == 'East':
            new_direction = direction
            new_snake, new_green = right(snake, green, red)
            if new_snake != snake:
                successors['ProdolzhiPravo'] = (new_snake, new_direction, new_green)

        elif direction == 'West':
            new_direction = direction
            new_snake, new_green = left(snake, green, red)
            if new_snake != snake:
                successors['ProdolzhiPravo'] = (new_snake, new_direction, new_green)
        ...
        if direction == 'South':
            ...
            new_snake, new_green = left(snake, green, red)
            if new_snake != snake:
                successors['SvrtiDesno'] = (new_snake, new_direction, new_green)
        ...
        if direction == 'South':
            ...
            new_snake, new_green = right(snake, green, red)
            if new_snake != snake:
                successors['SvrtiLevo'] = (new_snake, new_direction, new_green)
...
if __name__ == '__main__':
    ...
    m = int(input())
    red_apples = tuple(tuple(map(int, input().split(','))) for _ in range(m))

    initial = ((0, 9), (0, 8), (0, 7))
    direction = 'South'


    snake = Snake((initial, direction, green_apples), red_apples)
