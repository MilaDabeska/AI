...
class PriorityQueue(Queue):
    ...
    def __init__(self, order=min, f=lambda x: x):
        ...
        :param order: функција за подредување, ако order е min, се враќа елементот
                      со минимална f(x_position); ако order е max, тогаш се враќа елементот
                      со максимална f(x_position).
        :param f: функција f(x_position)
        """
...
def forward(x, y, direction):
    if direction == 'istok':
        if x < 9 and [x + 1, y] not in obstacles_list:
            x += 1
        return x, y, direction

    elif direction == 'zapad':
        if x > 0 and [x - 1, y] not in obstacles_list:
            x -= 1
        return x, y, direction

    elif direction == 'sever':
        if y < 9 and [x, y + 1] not in obstacles_list:
            y += 1
        return x, y, direction

    elif direction == 'zapad':
        if y > 0 and [x, y - 1] not in obstacles_list:
            y -= 1
        return x, y, direction


def backwards(x, y, direction):
    if direction == 'zapad':
        if x < 9 and [x + 1, y] not in obstacles_list:
            x += 1
            direction = 'istok'
...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        pac_x, pac_y, new_direct = forward(man_x, man_y, direction)
        if pac_x != man_x or pac_y != man_y:
            successors['ProdolzhiPravo'] = (pac_x, pac_y, new_direct,
                                            tuple([k for k in dots if k[0] != pac_x or k[1] != pac_y]))

        pac_x, pac_y, new_direct = backwards(man_x, man_y, direction)
        if pac_x != man_x or pac_y != man_y:
            successors['ProdolzhiNazad'] = (pac_x, pac_y, new_direct,
                                            tuple([k for k in dots if k[0] != pac_x or k[1] != pac_y]))

        pac_x, pac_y, new_direct = left(man_x, man_y, direction)
        if pac_x != man_x or pac_y != man_y:
            successors['SvrtiLevo'] = (pac_x, pac_y, new_direct,
                                       tuple([k for k in dots if k[0] != pac_x or k[1] != pac_y]))

        pac_x, pac_y, new_direct = right(man_x, man_y, direction)
        if pac_x != man_x or pac_y != man_y:
            successors['SvrtiDesno'] = (pac_x, pac_y, new_direct,
                                        tuple([k for k in dots if k[0] != pac_x or k[1] != pac_y]))
