...
class Disk(Problem):
    def __init__(self, l, n, goal=None):
        initial = []
        goal = []
        self.l = l
        self.n = n
    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            if i + 1 < L and state[i + 1] == 0:
                new_state = list(state)
                new_state[i] = 0
                new_state[i + 1] = disks
                successors['D1: Disk ' + str(disks)] = (tuple(new_state))

            if i + 2 < L and state[i + 1] != 0 and state[i + 2] == 0:
                new_state = list(state)
                new_state[i] = 0
                new_state[i + 2] = disks
                successors['D2: Disk ' + str(disks)] = (tuple(new_state))

            if i - 1 >= 0 and state[i - 1] == 0:
                new_state = list(state)
                new_state[i] = 0
                new_state[i - 1] = disks
                successors['L1: Disk ' + str(disks)] = (tuple(new_state))

            if i - 2 >= 0 and state[i - 1] != 0 and state[i - 2] == 0:
                new_state = list(state)
                new_state[i] = 0
                new_state[i - 2] = disks
                successors['L2: Disk ' + str(disks)] = (tuple(new_state))
...
if __name__ == '__main__':
    N = int(input()) #br na diskovi
    L = int(input()) #dimenzija na lenta

    final = Disk(L, N)
    print(breadth_first_graph_search(final).solution())
