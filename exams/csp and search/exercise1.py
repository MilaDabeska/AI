...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        if direction == 'istok':
            new_position = (x + 1, y, 'istok')
            if x + 1 < 10 and (new_position[0], new_position[1]) not in self.precki:
                new_points = points
                if (new_position[0], new_position[1]) in points:
                    new_points = tuple([x for x in points if x != (new_position[0], new_position[1])])
                succ['ProdolziPravo'] = (new_position, new_points)

            new_position = (x - 1, y, 'zapad')
            if x - 1 >= 10 and (new_position[0], new_position[1]) not in self.precki:
                new_points = points
                if (new_position[0], new_position[1]) in points:
                    new_points = tuple([x for x in points if x != (new_position[0], new_position[1])])
                succ['ProdolziNazad'] = (new_position, new_points)

            new_position = (x, y + 1, 'sever')
            if y + 1 < 10 and (new_position[0], new_position[1]) not in self.precki:
                new_points = points
                if (new_position[0], new_position[1]) in points:
                    new_points = tuple([x for x in points if x != (new_position[0], new_position[1])])
                succ['SvrtiLevo'] = (new_position, new_points)

            new_position = (x, y - 1, 'jug')
            if y - 1 >= 10 and (new_position[0], new_position[1]) not in self.precki:
                new_points = points
                if (new_position[0], new_position[1]) in points:
                    new_points = tuple([x for x in points if x != (new_position[0], new_position[1])])
                succ['SvrtiDesno'] = (new_position, new_points)
...
if __name__ == "__main__":
    ...
    for i in range(n):
        point = input()
        points_input.append((int(point.split(',')[0]), int(point.split(',')[1])))

    initial_state = ((x, y, d), tuple(points_input))
    pacman = Pacman(initial_state)
