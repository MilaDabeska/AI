...
class FootBall(Problem):
    ...
    def successor(self, state):
        ...
        # gore
        new_man_pos = (mx, my + 1)
        if my + 1 < self.grid_size[1] and new_man_pos not in self.obstacles:
            succ["Pomesti choveche gore"] = (new_man_pos, state[1])

        # dole
        new_man_pos = (mx, my - 1)
        if my - 1 >= 0 and new_man_pos not in self.obstacles:
            succ["Pomesti choveche dolu"] = (new_man_pos, state[1])

        # levo
        new_man_pos = (mx - 1, my)
        if mx - 1 >= 0 and new_man_pos not in self.obstacles:
            succ["Pomesti choveche levo"] = (new_man_pos, state[1])

        # desno
        new_man_pos = (mx + 1, my)
        if mx + 1 < self.grid_size[0] and new_man_pos not in self.obstacles:
