...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(5):
            ...
            tmp_list[i] = (x, y + 1)
            if self.check_valid(tmp_list):
                succ[f'Pomesti kvadratche {i + 1} gore'] = tuple(tmp_list)

            tmp_list[i] = (x, y - 1)
            if self.check_valid(tmp_list):
                succ[f'Pomesti kvadratche {i + 1} dole'] = tuple(tmp_list)

            tmp_list[i] = (x + 1, y)
            if self.check_valid(tmp_list):
                succ[f'Pomesti kvadratche {i + 1} desno'] = tuple(tmp_list)

            tmp_list[i] = (x - 1, y)
            if self.check_valid(tmp_list):
                succ[f'Pomesti kvadratche {i + 1} levo'] = tuple(tmp_list)
