...
if __name__ == "__main__":
    ...
    split = 1 - int(input()) / 100

    test_x = features[:int(len(features) * split)]
    test_y = classes[:int(len(classes) * split)]

    train_x = features[int(len(features) * split):]
    train_y = classes[int(len(classes) * split):]
