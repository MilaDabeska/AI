...
if __name__ == "__main__":
    col_index = int(input())
    features = [el[0:col_index] + el[col_index + 1:-1] for el in dataset]
    classes = [el[-1] for el in dataset]

    train_x = features[:int(len(features) * 0.85)]
    train_y = classes[:int(len(classes) * 0.85)]

    test_x = features[int(len(features) * 0.85):]
    test_y = classes[int(len(classes) * 0.85):]

    classifier = RandomForestClassifier(n_estimators=int(input()), criterion=input(), random_state=0)
    ...
    print(classifier.predict_proba([input])[0])
