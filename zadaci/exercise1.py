...
class Table(Problem):
    ...
    def successor(self, state):
        ...
        new_x, new_y = up_left(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'GoreLevo: (x={new_x},y={new_y})'] = (new_x, new_y)

        new_x, new_y = up_right(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'GoreDesno: (x={new_x},y={new_y})'] = (new_x, new_y)

        new_x, new_y = down_left(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'DoluLevo: (x={new_x},y={new_y})'] = (new_x, new_y)

        new_x, new_y = down_right(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'DoluDesno: (x={new_x},y={new_y})'] = (new_x, new_y)

        new_x, new_y = left(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'Levo: (x={new_x},y={new_y})'] = (new_x, new_y)

        new_x, new_y = right(x, y)
        if [x, y] != [new_x, new_y]:
            successors[f'Desno: (x={new_x},y={new_y})'] = (new_x, new_y)
...
if __name__ == "__main__":
    ...
    width = int(input())
    balls_position = ()
    number_obstacles = int(input())
    obstacles_position = ()

    for i in range(length,width):
        parts = input().split(',')
        balls_position += ((int(parts[0]), int(parts[1])))

    for i in range(number_obstacles):
        parts = input().split(',')
        obstacles_position += ((int(parts[0]), int(parts[1])))

    table = Table(length, width)
    print(breadth_first_graph_search(table).solution())
